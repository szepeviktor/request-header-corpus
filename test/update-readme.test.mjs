...
test('README navigation links follow the current manifest paths', async (context) => {
  ...
  for (const browser of ['chrome', 'edge', 'firefox', 'safari']) {
    for (const protocol of ['http1', 'http2']) {
      observations.push({
        ...
        request: {
          protocol,
          raw_file: `raw/${browser}/1/test/${protocol}/navigation-get.txt`,
        },
  ...
  assert.match(readme, /\[HTTP\/1\.1\]\(raw\/chrome\/1\/test\/http1\/navigation-get\.txt\)/);
  assert.match(readme, /\[HTTP\/2\]\(raw\/safari\/1\/test\/http2\/navigation-get\.txt\)/);
